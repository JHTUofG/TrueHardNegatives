...
def main(
    ...
    # dataset = pd.read_csv(data_dir, index=False, header=0, names=['query', 'doc_id_a', 'doc_id_b'])
    dataset = pd.read_csv(f'data/{dataset_name}.csv', index_col=0)
    train = pd.read_csv(f'data/{train_name}.csv', index_col=0)
    ...
    train_iter = iterate(train, style='t5') #_logger.pbar(iterate(train, style='t5'), desc='total train samples')
    step = 0 
    total_loss = 0
